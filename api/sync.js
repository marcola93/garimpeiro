...
async function syncOrders(accessToken, userId, dateFrom) {
  ...
  let offset = 0;
  const limit = 50;
  const totalRes = await fetch(`https://api.mercadolibre.com/orders/search?seller=${userId}&order.date_created.from=${encodeURIComponent(dateFrom)}&sort=date_desc&limit=1`, {
    headers: { 'Authorization': `Bearer ${accessToken}` }
  });
  if (!totalRes.ok) return { synced: 0, error: totalRes.status };
  const totalData = await totalRes.json();
  const totalOrders = totalData.paging?.total || 0;

  // Pagina todos os pedidos do período
  while (offset < totalOrders && allOrders.length < 1000) {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Usa formato UTC para a API do ML
    const d = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
    const dateFrom = d.toISOString().split('.')[0] + '.000Z';
