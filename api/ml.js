...
module.exports = async (req, res) => {
  ...
  // POST /api/ml?action=save_verifier — salva verifier no store
  if (req.method === 'POST' && req.query.action === 'save_verifier') {
    let body = '';
    ...
    return res.status(400).json({ error: 'missing verifier or state' });
  }

  // GET /api/ml?action=get_verifier&state=xxx
  if (req.query.action === 'get_verifier') {
